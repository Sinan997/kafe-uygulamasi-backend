...
const addCategory = async (req, res) => {
  ...
  const isTitleAvailable = await Category.findOne({ title });

  if (isTitleAvailable) {
    return res.status(406).json({ message: 'Bu İsimde Kategori Bulunmakta', success: false });
  }

  const index = (await Category.find()).length;
  ...
  try {
    ...
    return res
      .status(201)
      .json({ message: 'Kategori Oluşturuldu', success: true, category: newCategory });
  } catch (error) {
    console.log(error);
...
const getAllProducts = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ message: 'Ürünler Başarıyla Getirildi', success: true, products, category });
  } catch (error) {
    console.log(error);
    return res.status(404).json({ message: 'Ürünler Getirilirken Hata Oluştu', success: false });
  }
